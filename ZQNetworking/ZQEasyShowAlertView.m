...
@interface ZQEasyShowAlertItem : NSObject
...
@implementation ZQEasyShowAlertItem
...
@interface ZQEasyShowAlertView()<CAAnimationDelegate>
...
@property (nonatomic,strong)NSMutableArray<ZQEasyShowAlertItem *> *alertItemArray ;
...
@implementation ZQEasyShowAlertView
...
+ (instancetype)showAlertWithType:(alertShowType)type title:(NSString *)title message:(NSString *)message
{
    ...
    ZQEasyShowAlertView *showView = [[ZQEasyShowAlertView alloc]initWithFrame:[UIScreen mainScreen].bounds];
...
- (void)addItemWithTitle:(NSString *)title itemType:(ShowAlertItemType)itemType callback:(alertItemCallback)callback
{
    ...
    ZQEasyShowAlertItem *item = [[ZQEasyShowAlertItem alloc]init];
    item.title = title ;
...
- (void)systemShow
{
    ...
    [self.alertItemArray enumerateObjectsUsingBlock:^(ZQEasyShowAlertItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [[ZQEasyShowConfig topViewController] presentViewController:alertC animated:YES completion:nil];
}
...
{
    CGFloat bgViewMaxWidth = self.alertShowType==alertShowTypeAlert ?  kScreen_Width*0.75 : kScreen_Width ;
    ...
    CGFloat actionShowAddSafeHeiht = self.alertShowType==alertShowTypeActionSheet ? kSafeBottomMargin : 0 ;
    ...
    switch (self.alertShowType) {
        ...
        {
            self.alertBgView.center = CGPointMake(kScreen_Width/2, kScreen_Height-(totalHeight/2));
        }break ;
        default:
            break;
    }
...
{
    ZQEasyShowAlertItem *item = self.alertItemArray[button.tag];
    if (item.callback) {
        item.callback(self);
    }
    [self alertWindowTap];
...
{
    ZQEasyShowAlertItem *item = self.alertItemArray[index];
    ...
    UIImage *bgImage = [ZQEasyShowConfig imageWithColor:[UIColor whiteColor]];
    UIImage *bgHighImage = [ZQEasyShowConfig imageWithColor:[[UIColor whiteColor]colorWithAlphaComponent:0.7] ];
...
- (void)showEndAnimationWithType:(alertAnimationType)type completion:(void(^)(void))completion
{
    if (self.alertShowType == alertShowTypeActionSheet) {
        [UIView animateWithDuration:ZQEasyShowAnimationTime animations:^{
            self.alertBgView.top = kScreen_Height ;
    ...
    switch (type) {
        ...
        {
            [UIView animateWithDuration:ZQEasyShowAnimationTime
                                  delay:0
...
- (void)showStartAnimationWithType:(alertAnimationType)type completion:(void(^)(void))completion
{
    if (self.alertShowType == alertShowTypeActionSheet) {
        self.alertBgView.top = kScreen_Height ;
        [UIView animateWithDuration:ZQEasyShowAnimationTime animations:^{
            self.alertBgView.top = (kScreen_Height-self.alertBgView.height)-5 ;
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:0.05 animations:^{
                self.alertBgView.top = (kScreen_Height-self.alertBgView.height) ;
            } completion:^(BOOL finished) {
            }];
        }];
        return ;
    ...
    switch (type) {
        case alertAnimationTypeFade:
        {
            ...
            [UIView setAnimationDuration:ZQEasyShowAnimationTime];
        ...
        case alertAnimationTypePush:
        {
            self.alertBgView.top = kScreen_Height ;
            [UIView animateWithDuration:ZQEasyShowAnimationTime animations:^{
                self.alertBgView.top = (kScreen_Height-self.alertBgView.height)/2-5 ;
            } completion:^(BOOL finished) {
                [UIView animateWithDuration:0.05 animations:^{
                    self.alertBgView.top = (kScreen_Height-self.alertBgView.height)/2 ;
                } completion:^(BOOL finished) {
                }];
            }];
        }break ;
...
- (UILabel *)alertTitleLabel
{
    if (nil == _alertTitleLabel) {
        _alertTitleLabel = [[ZQEasyShowLabel alloc] initWithContentInset:UIEdgeInsetsMake(35, 30, 15, 30)];
...
- (UILabel *)alertMessageLabel
{
    if (nil == _alertMessageLabel) {
        _alertMessageLabel = [[ZQEasyShowLabel alloc] initWithContentInset:UIEdgeInsetsMake(15, 30, 20, 30)];
