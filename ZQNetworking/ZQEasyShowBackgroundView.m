...
@interface ZQEasyShowBackgroundView()
...
@implementation ZQEasyShowBackgroundView
    ...
    if ([super initWithFrame:frame]) {
        ...
        if ((!(self.isShowedStatusBar||self.isShowedNavigation))) {
            ...
            if (_showTextStatus != ShowTextStatusPureText && (!ISEMPTY_S(text))) {//只要不是纯文字，其他的都需要显示图片
                self.imageView.top  = ZQEasyDrawImageEdge ;
            }
        }
        ...
        if (!ISEMPTY_S(text)) {
            CGSize textSize = [ZQEasyShowConfig textWidthWithStirng:text
...
- (UIWindow *)showTextWindow
{
    if (nil == _showTextWindow) {
        CGFloat showHeight = self.isShowedStatusBar ? kStatus_Height : kNavNormalHeight+kStatus_Height ;
        _showTextWindow = [[UIWindow alloc]initWithFrame:CGRectMake(0, -showHeight , kScreen_Width, showHeight )];
...
{
    if (nil == _imageView) {
        CGFloat imageWH = ZQEasyDrawImageWH ;
        CGFloat imageX = (self.width-ZQEasyDrawImageWH)/2 ;
        CGFloat imageY = ZQEasyDrawImageEdge/2 ;
        if (self.isShowedStatusBar) {
            ...
            imageX = 10 ;
            imageY = (self.height - imageWH)/2 + (kMatchiPhoneXSeries ? 20 : 3 );
        }
