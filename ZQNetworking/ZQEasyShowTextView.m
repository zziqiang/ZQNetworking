...
@interface ZQEasyShowTextView()<CAAnimationDelegate>
...
@property (nonatomic,strong)ZQEasyShowBackgroundView *showBgView ;//用于放图片和文字的背景
...
@implementation ZQEasyShowTextView
...
- (void)showViewWithSuperView:(UIView *)superView
{
    ...
    if (self.options.textSuperViewReceiveEvent) {//父视图能接受事件
        //self的大小为显示区域的大小
        [self setFrame:CGRectMake((kScreen_Width-showFrame.size.width)/2, showFrame.origin.y, showFrame.size.width, showFrame.size.height)];
    ...
    self.showBgView = [[ZQEasyShowBackgroundView alloc]initWithFrame:showFrame
    ...
    if (self.options.textShadowColor && self.options.textShadowColor!=[UIColor clearColor]) {
        CGFloat afterStart = self.options.textAnimationType==TextAnimationTypeBounce ? ZQEasyShowAnimationTime : ZQEasyShowAnimationTime/2 ;
...
//                }];
        ...
        case TextAnimationTypeBounce:
        {
            if ( (self.isShowedStatusBar || self.isShowedNavigation)) {
                ...
                [self.showBgView showStartAnimationWithDuration:ZQEasyShowAnimationTime];
            }
    ...
    switch (self.options.textAnimationType) {
        ...
        case TextAnimationTypeBounce:
        {
            ...
            if ( (self.isShowedStatusBar || self.isShowedNavigation)) {
                ...
                [self.showBgView showEndAnimationWithDuration:ZQEasyShowAnimationTime];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(ZQEasyShowAnimationTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [self removeFromSuperview];
                });
            }
        }
                   ...
                   stauts:(ShowTextStatus)status
{
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            ZQEasyShowTextView *showView = (ZQEasyShowTextView *)subview ;
            [showView removeSelfFromSuperView];
        }
    }
    
    ZQEasyShowTextView *showView = [[ZQEasyShowTextView alloc] initWithFrame:CGRectZero];
    ...
    if (showView.showTime > ZQTextShowMaxTime) {
        showView.showTime = ZQTextShowMaxTime ;
    }
...
{
    //显示图片的高度。
    CGFloat imageH = self.showTextStatus==ShowTextStatusPureText ?:(ZQEasyDrawImageWH + ZQEasyDrawImageEdge) ;
    
    //显示区域的宽高
    CGFloat backGroundH = 0 ;
    CGFloat backGroundW = kScreen_Width;
    switch (self.options.textStatusType) {
        case ShowTextStatusTypeStatusBar://如果是在statusbar上，则高固定，不需要计算
            backGroundH = kStatus_Height ;
            break;
        case ShowTextStatusTypeNavigation:
            backGroundH = kStatus_Height + kNavNormalHeight ;
            break ;
        default:{
            CGSize textSize = CGSizeZero ;
            if (!ISEMPTY_S(self.showText)) {
                textSize = [ZQEasyShowConfig textWidthWithStirng:self.showText
                                                         font:self.options.textTitleFount
                                                     maxWidth:ZQTextShowMaxWidth];
            }
            backGroundH = (textSize.height?(textSize.height+30):0) + imageH ;
            backGroundW = textSize.width?(textSize.width+40):0  ;
            
            if (backGroundW < ZQEasyShowViewHeaderMinWidth) {
                backGroundW = ZQEasyShowViewHeaderMinWidth  ;
            }
        } break;
    }
    
    //显示区域的y值
    CGFloat showFrameY = (kScreen_Height-backGroundH)/2  ;//默认显示在中间
    ...
    switch (self.options.textStatusType ) {
        ...
        case ShowTextStatusTypeTop:
            showFrameY = kStatus_Height + kNavNormalHeight  + ZQEasyTextShowEdge ;
            break;
        case ShowTextStatusTypeBottom:
            showFrameY = kScreen_Height - backGroundH - ZQEasyTextShowEdge ;
            break ;
        default: break;
    }
    //    }
